...
from PyQt5.QtCore import QSize,pyqtSlot,Qt,PYQT_VERSION_STR,QT_VERSION_STR,QSettings,QTimer,QFile, QFileInfo,QByteArray,QVariant
...
class MainWindow(QMainWindow):
    ...
    def editMirrorHorizontal(self, on):
        if self.image.isNull():
            return
        #水平镜像，竖直镜像标志（可以是无镜像、竖直镜像或水平镜像，不能既是水平又是竖直）
        self.image = self.image.mirrored(True, False)
        self.showImage()
        self.mirroredhorizontally = not self.mirroredhorizontally#是否是水平镜像
        self.dirty = True
    ...
    def closeEvent(self, event):
        if self.okToContinue():
            ...
            recentFiles=QVariant((self.recentFiles)if self.recentFiles is not None else QVariant())
            # print(filename.value(),recentFiles.value())
            settings.setValue("LastFile", filename.value())
            settings.setValue("RecentFiles", recentFiles.value())
